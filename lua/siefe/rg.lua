...
function M.ripgrepfzf(fullscreen, dir, kwargs)
  ...
  local function reopen(selected, changes)
    ...
    if changes then
      for k, v in pairs(changes) do kwargs[k] = v end
    end
  ...
  actions[config.rg_history_key] = function(selected, _opts)
    ...
    if kwargs.files then
      ...
      if vim.deep_equal(kwargs.paths, recent) then
        kwargs.paths = {}
        vim.schedule(function() M.ripgrepfzf(fullscreen, kwargs.orig_dir, kwargs) end)
      else
        kwargs.paths = recent
        vim.schedule(function()
  ...
  if mode == 'rg' then
    ...
    local source = mode == 'files' and files_cmd
                or string.format(cmd_fmt, vim.fn.shellescape(kwargs.query))
