...
local function highlight_keys(str)
  ...
  str = str:gsub('<[^ >]+>', function(m)
    if m == '<Plug>' then return utils.blue('<Plug>', 'SpecialKey') end
...
function M.maps(fullscreen, query, modes)
  ...
  for _, m in ipairs(filtered) do
    if #(m.lhs or '') > max_len then max_len = #(m.lhs or '') end
  end

  local ordinary = vim.tbl_filter(function(m) return (m.sid or 0) >= 0 end, filtered)
  local special  = vim.tbl_filter(function(m) return (m.sid or 0) < 0 end, filtered)
  ...
  for _, m in ipairs(ordinary) do
    ...
    local line  = highlight_keys(
      m.mode .. '•' .. file .. '•' .. (m.lnum or 0) .. '•' .. (m.mode or '') .. '•' .. (m.lhs or '') .. '•'
      .. utils.red(m.mode or '') .. ' '
      .. (m.lhs or '')
  ...
  for _, m in ipairs(special) do
    ...
    local line  = highlight_keys(
      m.mode .. '•' .. desc .. '•' .. (m.lnum or 0) .. '•' .. (m.mode or '') .. '•' .. (m.lhs or '') .. '•'
      .. utils.red(m.mode or '') .. ' '
      .. (m.lhs or '')
      .. string.rep(' ', max_len - #(m.lhs or '') + 1)
      .. (m.rhs or '')
