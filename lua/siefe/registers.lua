...
local function get_all_registers()
  local regnames = {}
  for i = string.byte('a'), string.byte('z') do table.insert(regnames, string.char(i)) end
  for i = string.byte('0'), string.byte('9') do table.insert(regnames, string.char(i)) end
...
local function regsave()
  ...
  if not file or not reg then return end
...
function M.registers(fullscreen, kwargs)
  ...
  actions['default'] = { fn = function(selected, opts)
    ...
    vim.fn.append(0, '### Editing register `' .. reg .. '`. Add control characters by preceding them with `ctrl-v` ###')
    vim.cmd('put ' .. reg)
    vim.cmd('2delete _')
