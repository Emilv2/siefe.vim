...
function M.gitlogfzf(fullscreen, kwargs)
  ...
  -- Build flags
  local branches   = kwargs.branches == '--all' and '--all '
                  or (kwargs.branches ~= '' and (kwargs.branches .. ' ') or '')
  local notbranches= kwargs.notbranches ~= '' and (kwargs.notbranches .. ' ') or ''
  local authors    = table.concat(vim.tbl_map(function(a)
  ...
  if #kwargs.type > 0 and #kwargs.paths > 0 then
    ...
    for _, path in ipairs(kwargs.paths) do
      if path:sub(-1) == '/' then
        for _, t in ipairs(kwargs.type) do table.insert(p, vim.fn.shellescape(path .. t)) end
      else
  ...
  elseif #kwargs.type > 0 then
    ...
    paths_str = table.concat(vim.tbl_map(function(p)
  ...
  if #kwargs.line_range > 0 then
    ...
    initial_command = 'git log --no-patch -z -L' .. kwargs.line_range[1] .. ',' .. kwargs.line_range[2] .. ':' .. paths_str
      .. ' ' .. branches .. notbranches .. authors .. regex .. ignore_case
    ...
    reload_command = logger .. write_reload .. logger
  ...
  if vim.fn.executable('grepdiff') == 1 then
    p3 = ' bash -c \''
      ...
      .. git_SG .. ' -C ' .. git_root_cmd .. ' -c diff.external=' .. utils.bin_path('pickaxe-diff')
  ...
  actions['default'] = function(selected, opts)
    ...
    local qf = vim.tbl_map(function(line)
      ...
      return { bufnr = vim.fn.bufadd(vim.trim(vim.fn.system('git show -s --format=%H ' .. h))),
               text = msg:sub(1, vim.o.columns - 10), module = h }
    end, items)
    vim.cmd('Gedit ' .. hash)
  ...
  actions[config.gitlog_branch_key] = function(selected, opts)
    ...
    gb.branch_select(function(blines)
      ...
      elseif blines[1] == config.branches_all_key then
        ...
        kwargs.branches = table.concat(vim.tbl_map(function(l)
  ...
  actions[config.gitlog_not_branch_key] = function(selected, opts)
    ...
    gb.branch_select(function(blines)
      if blines[1] == config.abort_key then
        ...
        kwargs.notbranches = table.concat(vim.tbl_map(function(l)
          return '^' .. vim.trim(vim.split(l, ':')[1])
        end, vim.list_slice(blines, 2)), ' ')
      end
      M.gitlogfzf(fullscreen, kwargs)
