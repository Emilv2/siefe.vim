...
local function format_buffer(b, git_dir)
  ...
  return vim.trim(string.format('%s//%d//[%s] %s\t%s%s\t%s',
    name, line, utils.yellow(tostring(b), 'Number'), flag, rel_name, extra, line_text))
end
...
function M.buffers(fullscreen, kwargs)
  ...
  local function get_query(selected, opts)
    ...
    if selected and #selected > 0 and not selected[1]:match('%[%d') then return selected[1] end
    ...
    for _, line in ipairs(items) do
      ...
      if b then
        ...
        elseif modified and not readonly then
          local action = vim.fn.input('buffer "' .. vim.fn.bufname(b) .. '" modified. Save, discard or abort? (s/d/a) ')
          if action == 's' then
        ...
        elseif modified and readonly then
          local action = vim.fn.input('buffer "' .. vim.fn.bufname(b) .. '" modified (readonly). Discard or abort? (d/a) ')
          if action == 'd' then vim.cmd('bdelete! ' .. b) end
        end
      end
    end
